 ...
 */
fun <T> UiState<T>.copyWithResult(value: Res<T>): UiState<T> {
    return when (value) {
        is Res.Success -> copy(loading = false, exception = null, data = value.data)
        is Res.Error -> copy(loading = false, exception = value.exception)
    }
}
